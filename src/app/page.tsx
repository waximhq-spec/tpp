  ...
  return (
    <div className="bg-background text-on-background min-h-screen font-sans flex flex-col selection:bg-secondary/20 selection:text-on-secondary-container pb-16 md:pb-0">
      ...
      <a
        ...
        className="fixed bottom-20 md:bottom-6 right-4 md:right-6 z-40 bg-[#25D366] hover:bg-[#20ba5a] text-white p-3 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 flex items-center justify-center cursor-pointer"
      ...
      {/* Mobile Sticky CTA */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-100 px-4 py-2.5 shadow-[0_-2px_12px_rgba(0,0,0,0.04)] flex items-center justify-between gap-3">
        <div>
          <p className="text-[9px] uppercase font-bold tracking-widest text-accent">Subsidy Active</p>
          <p className="text-xs font-extrabold text-primary">Get ₹78,000 Off</p>
        </div>
        <a
          href="tel:+917889880188"
          className="bg-primary text-white font-semibold text-xs px-5 py-2.5 rounded-xl transition-all shadow-md shrink-0 cursor-pointer text-center"
        >
          Free Consultation
        </a>
      </div>
